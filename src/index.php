...
$tasas = [
    'USD' => ['nombre' => 'Dólar (USD)', 'tasa' => 1.00],
    'EUR' => ['nombre' => 'Euro (EUR)', 'tasa' => 0.92],
    'COP' => ['nombre' => 'Peso Colombiano (COP)', 'tasa' => 4050.00],
    'MXN' => ['nombre' => 'Peso Mexicano (MXN)', 'tasa' => 18.50],
    'PEN' => ['nombre' => 'Sol Peruano (PEN)', 'tasa' => 3.75],
    'ARS' => ['nombre' => 'Peso Argentino (ARS)', 'tasa' => 950.00],
    'GBP' => ['nombre' => 'Libra Esterlina (GBP)', 'tasa' => 0.79]
];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($accion === 'convertir') {
        ...
        if ($monto > 0 && isset($tasas[$origen]) && isset($tasas[$destino])) {
            ...
            $montoDestino = $monto * $tasaEfectiva;
                ...
                <div class="card-body p-4">
                    <form method="POST" action="">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="number" step="0.01" name="monto" class="form-control" placeholder="Ej: 100" required>
                        </div>
                        ...
                        <button type="submit" class="btn btn-primary w-100 py-2 fw-semibold">Convertir</button>
                    </form>
                    ...
                    <?php if ($resultado): ?>
                        <div class="alert alert-success mt-4 mb-0 text-center">
                            ...
                            <small class="text-muted">Tasa de cambio: 1 <?= $resultado['origen'] ?> = <?= number_format($resultado['tasa'], 4) ?> <?= $resultado['destino'] ?></small>
                        </div>
                    <?php endif; ?>
                </div>
            </div>
                ...
                <div class="card-body p-4">
                    <div class="d-flex justify-content-between align-items-center mb-3">
                        ...
                        <?php if (!empty($historial)): ?>
                            <form method="POST" action="" onsubmit="return confirm('¿Desea limpiar el historial?');">
                    ...
                    <?php if (empty($historial)): ?>
                        <p class="text-muted text-center mb-0">No hay conversiones registradas todavía.</p>
                    <?php else: ?>
